...
public class Waffle extends Application {
    ...
    private static double X_OFFSET = 50;
    //Y_OFFSET moves the board on the y-axis out of the left upper corner.
    private static double Y_OFFSET = 100;
    //initialize the X of the first rectangle
    private static double X_RECTANGLE = X_OFFSET;
    //initialize the Y of the first rectangle
    private static double Y_RECTANGLE = Y_OFFSET;
   ...
   public static void calculateAmount() {
    ...
    /*
     * method to draw the 100 rectangles
     */
    public static void drawChart(Group group) {
        for (int i = 0; i < amounts.length; i++) {
            for (int j = 1; j <= amounts[i]; j++) {
                ...
                X_RECTANGLE += WIDTH+SPACING;
                if (X_RECTANGLE > X_OFFSET + 9 * (WIDTH+SPACING)) {
                    X_RECTANGLE = X_OFFSET;
                    Y_RECTANGLE += HEIGHT+SPACING;
                }
            }
        }
    }
    ...
    /*
     * method to display the legend of the chart
     */
   public static void drawLegend(Group group) {
        for (int i = 0; i < MAXIMUM; i++) {
            Rectangle rectangle = new Rectangle(X_OFFSET + 9 * (WIDTH + SPACING) + WIDTH*3, Y_OFFSET + (HEIGHT + SPACING) * (i + 1), WIDTH, HEIGHT);
            rectangle.setFill(COLORS[i]);
            Text text = new Text(X_OFFSET + 9 * (WIDTH + SPACING)  + WIDTH*4.5, Y_OFFSET + (HEIGHT + SPACING) * (i + 1)+ HEIGHT*4/5, texts[i]);
            //Write in black in a particular font.
            text.setFont(Font.font("Arial", WIDTH/1.5 ));
            group.getChildren().add(rectangle);
    ...
    /**
     *  We create a frame with a panel on which the Waffle chart is displayed.
